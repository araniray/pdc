...
double my_pi, pi_sum, pi_est, mean_pi, err; int task_id, n_tasks, rc, i;
MPI_Status status;
...
mean_pi = ( (mean_pi * i) + pi_est ) / (i + 1); // running average err = mean_pi - 3.14159265358979323846;
printf("%d throws: mean_pi %.12f: error %.12f\n",
             (NDARTS * (i + 1)), mean_pi, err);
} }
if (task_id == MASTER)
printf ("PS, the real value of pi is about
